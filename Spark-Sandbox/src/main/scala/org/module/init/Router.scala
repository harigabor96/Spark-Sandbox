...
object Router {
  ...
  def executePipeline(spark: SparkSession, conf: Conf): Unit = conf.pipeline() match {
    case "sandbox-pipeline" =>
      bronzesilvergold.tables.sandbox_table.Pipeline(spark, conf.rawZonePath(), conf.curatedZonePath()).execute()
    case "sandbox-query" =>
      platinum.queries.sandbox_query.Query(spark, conf.curatedZonePath()).execute().show(false)
    case _ =>
